...
@Tag(name = "用户行为表", description = "用户行为表接口")
public class NewsUserActionController extends BladeController {
	...
	@Operation(summary = "详情", description  = "传入newsUserAction")
	...
	@Operation(summary = "分页", description  = "传入newsUserAction")
	...
	@Operation(summary = "新增", description  = "传入newsUserAction")
	...
	@Operation(summary = "修改", description  = "传入newsUserAction")
	...
	@Operation(summary = "新增或修改", description  = "传入newsUserAction")
	...
	@Operation(summary = "导出数据", description  = "传入newsUserAction")
	public void exportNewsUserAction(@Parameter(hidden = true) @RequestParam Map<String, Object> newsUserAction, BladeUser bladeUser, HttpServletResponse response) {
		...
		//	queryWrapper.lambda().eq(NewsUserAction::getTenantId, bladeUser.getTenantId());
		//}
		//queryWrapper.lambda().eq(NewsUserActionEntity::getIsDeleted, BladeConstant.DB_NOT_DELETED);
		List<NewsUserActionExcel> list = newsUserActionService.exportNewsUserAction(queryWrapper);
		ExcelUtil.export(response, "用户行为表数据" + DateUtil.time(), "用户行为表数据表", list, NewsUserActionExcel.class);
	}
